...
document.getElementById("practiceCont").style.display = "none"
//document.getElementById("czechPart").style.display = "none"
...
allCont.classList.add("noWords")

allCont.textContent = "No words added to this folder yet!"
...
if(oneOrTwo === 0){
  if(emptyStrings < 6){
      ...
      for (let i = 0, len = caseStrList.length; i < len; i++){
        ...
        var declension = caseStrList[caseNumber]
        }
...
if(oneOrTwo === 1){
      ...
      for (let i = 0, len = caseStrListPl.length; i < len; i++){
        ...
        var declension = caseStrListPl[caseNumber]
        }
...
if(emptyStrings >= 6 && oneOrTwo === 0){
  ...
  if(userAnswer === correctAnswer){
    ...
    czechPart.classList.add("green")

    answersContainer.classList.add("roi")


      var incompSent = czechPart.textContent
      var completeSent = incompSent.replace("...", " " + userAnswer + " ")

      czechPart.textContent = completeSent

  }else{
    var incorrectNotif = document.getElementById("incorrectNotif")
    incorrectNotif.style.opacity = "1"

    if(writtenAnsCont.style.display === "block"){
      incorrectNotif.textContent = "Wrong! The correct answer is " + correctAnswer + ". Try again!"
    }
...
function nextQuestion(){
        ...
        answersContainer.classList.remove("roi")
      ...
      for (let i = 0, len = answerOption.length; i < len; i++){
        ...
        czechPart.style.transform = "translateX(100%)";
          ...
          setTimeout(function(){
          czechPart.classList.remove("green")
